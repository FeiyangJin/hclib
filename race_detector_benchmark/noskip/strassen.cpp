...
static unsigned int POWER = 4;
static unsigned int DAC_ARITH_BASECASE = (1 << POWER);  // 16*16
static unsigned int MATMUL_THRESH = (1 << (POWER + 4)); // 256*256
...
void mm_dac_z(hclib::promise_t<void> *output_dep, REAL *C, REAL *A, REAL *B, int n, bool add);
...
void mm_dac_z(hclib::promise_t<void> *output_dep, REAL *C, REAL *A, REAL *B, int n, bool add) {
    ...
    ds_hclib_ready(true);
  #endif
  if (output_dep) output_dep->get_future()->wait();
  ...
  hclib::async([&p1, &C11, A11, B11, n, add](){
    mm_dac_z(NULL, C11, A11, B11, n>>1, add);
    p1->put();
  });
    ...
    ds_hclib_ready(false);
  #endif
  hclib::async([&p2, &C12, A11, B12, n, add](){
    mm_dac_z(NULL, C12, A11, B12, n>>1, add);
    p2->put();
  });
    ...
    ds_hclib_ready(false);
  #endif
  hclib::async([&p3, &C21, A21, B11, n, add](){
    mm_dac_z(NULL, C21, A21, B11, n>>1, add);
    p3->put();
  });
    ...
    ds_hclib_ready(false);
  #endif
  hclib::async([&p4, &C22, A21, B12, n, add](){
    mm_dac_z(NULL, C22, A21, B12, n>>1, add);
    p4->put();
  });
  ...
  hclib::finish([&](){
    ...
    hclib::async([p1, &C11, A12, B21, n](){
      // this is 355
      mm_dac_z(p1, C11, A12, B21, n >> 1, true);
      // mm_dac_z(NULL, C11, A12, B21, n >> 1, true);
    });

    #ifdef RACE_DETECTION
      ds_hclib_ready(false);
    #endif
    hclib::async([p2, &C12, A12, B22, n](){
      mm_dac_z(p2, C12, A12, B22, n >> 1, true);

      // mm_dac_z(NULL, C12, A12, B22, n >> 1, true);
    });

    #ifdef RACE_DETECTION
      ds_hclib_ready(false);
    #endif
    hclib::async([p3, &C21, A22, B21, n](){

      mm_dac_z(p3, C21, A22, B21, n >> 1, true);
      // mm_dac_z(NULL, C21, A22, B21, n >> 1, true);
    });

    #ifdef RACE_DETECTION
      ds_hclib_ready(false);
    #endif
    mm_dac_z(p4, C22, A22, B22, n >> 1, true);
    // mm_dac_z(NULL, C22, A22, B22, n >> 1, true);
...
static void final_add(REAL *C11, REAL *C12, REAL *C21, REAL *C22, REAL *M2, REAL *M5, REAL *T1, int n, hclib::promise_t<void> **dependences);
static void final_add(REAL *C11, REAL *C12, REAL *C21, REAL *C22, 
                      REAL *M2, REAL *M5, REAL *T1, int n, hclib::promise_t<void> **dependences) {
  ...
  hclib::finish([&](){
    ...
    #endif
    hclib::async([&](){
      final_add(C11_0, C12_0, C21_0, C22_0, M2_0, M5_0, T1_0, n >> 1, NULL);
    });
    ...
    #endif
    hclib::async([&](){
      final_add(C11_1, C12_1, C21_1, C22_1, M2_1, M5_1, T1_1, n >> 1, NULL);
    });
    ...
    #endif
    hclib::async([&](){
      final_add(C11_2, C12_2, C21_2, C22_2, M2_2, M5_2, T1_2, n >> 1, NULL);
    });
...
static void setup_add(REAL *S1, REAL *S2, REAL *S3, REAL *S4,
  ...
  hclib::finish([&](){
    ...
    #endif
    hclib::async([&](){
      setup_add(S1_0,S2_0,S3_0,S4_0,S5_0,S6_0,S7_0,S8_0,
    ...
    #endif
    hclib::async([&](){
      setup_add(S1_1,S2_1,S3_1,S4_1,S5_1,S6_1,S7_1,S8_1,
    ...
    #endif
    hclib::async([&](){
      setup_add(S1_2,S2_2,S3_2,S4_2,S5_2,S6_2,S7_2,S8_2,
...
void strassen_z(REAL *C, REAL *A, REAL *B, int n) {
  ...
  setup_add(S1,S2,S3,S4,S5,S6,S7,S8,A11,A12,A21,A22,B11,B12,B21,B22, new_n);
    ...
    ds_hclib_ready(false);
  #endif
  hclib::promise_t<void> *phandles[7];
  for(int i=0; i<7; i++){
    phandles[i] = new hclib::promise_t<void>();
  }
