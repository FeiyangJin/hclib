...
public class TestStreamCluster2 extends Test
{
  static {
    theClass = TestStreamCluster2.class;
  }
  ...
  private Channel<Integer> pspeedy_open;
  private ChannelReader<Integer> pspeedy_open_f;
  private CheckedCompletableFuture [] pspeedy_costs = new CheckedCompletableFuture[NPROC];
  private CheckedCompletableFuture<Double> pspeedy_totalcost;
  ...
  private double pspeedy (double z, int pid)
  {
    ...
    if (pid==0) {
      kcenter = 1;
      pspeedy_open = new Channel<Integer>();
      pspeedy_open_f = pspeedy_open.fromLatest();
      pspeedy_totalcost = new CheckedCompletableFuture<>();
    }

    pspeedy_costs[pid] = new CheckedCompletableFuture<Double>();

    barrier.getParty(pid).arrive();

    if (pid != 0) {
      // we are not the master threads. we wait until a center is opened.
      for (Integer opened : pspeedy_open_f) {
        for (int k = k1; k < k2; k++) {
          double distance = dist(p[opened], p[k], DIM);
      ...
      for (int i = 1; i < p.length; i++) {
        boolean to_open = rand.nextDouble() < (p[i].cost/z);
        if (to_open) {
          kcenter++;
          pspeedy_open.put(i);

          for (int k = k1; k < k2; k++) {
            double distance = dist(p[i], p[k], DIM);
            if (distance*p[k].weight < p[k].cost) {
              p[k].cost = distance * p[k].weight;
              p[k].assign = i;
            }
      ...
      pspeedy_open.terminate();
    }

    double mytotal = 0;
    for (int k = k1; k < k2; k++)
      mytotal += p[k].cost;
    pspeedy_costs[pid].complete(mytotal);

    // aggregate costs from each thread
    if (pid == 0) {
      double totalcost = z*kcenter;
      for (int j = 0; j < NPROC; j++)
        totalcost += (Double) pspeedy_costs[j].join();
      pspeedy_totalcost.complete(totalcost);
    }

    return pspeedy_totalcost.join();
  }
  ...
  private Channel<Void> p_masterCondition;
  private ChannelReader<Void> p_masterCondition_f;
  ...
  private CheckedCompletableFuture<Double> pgain_gl_cost_of_opening_x;

  private double pgain (int x, double z, int pid, Iterator<Void> masterCondition)
  {
    //my block
    int bsize = p.length/NPROC;
    int k1 = bsize * pid;
    ...
    if (pid == 0) {
      ...
      p_masterCondition.put(null);
    } else
      masterCondition.next();
    ...
    */
    ...
    if (pid == 0)
      pgain_gl_cost_of_opening_x = new CheckedCompletableFuture<>();
    ...
    if (pid == 0) {
      ...
      for (int p = 0; p < NPROC; p++) {
        ...
        gl_cost_of_opening_x += pgain_work_mem[p*stride+K+1];
      }
      pgain_gl_cost_of_opening_x.complete(gl_cost_of_opening_x);
    }

    // Now, check whether opening x would save cost; if so, do it, and
    // otherwise do nothing

    if (pgain_gl_cost_of_opening_x.join() < 0 ) {
    ...
    return -pgain_gl_cost_of_opening_x.join();
  }
  ...
  private double pFL (double z, double cost, int iter, double e, int pid,
                      Iterator<Void> masterCondition)
  {
    double change = cost;
    ...
    while (change/cost > 1.0*e) {
      ...
      for (int i = 0; i < iter; i++) {
        int x = i % pkmedian_feasible.length;
        change += pgain(pkmedian_feasible[x], z, pid, masterCondition);
      }
      cost -= change;
    }
  ...
  private void selectfeasible_fast (int kmin, int pid)
  {
  ...
  private CheckedCompletableFuture [] pkmedian_hizs = new CheckedCompletableFuture[NPROC];
  ...
  private void pkmedian (int kmin, int kmax, int pid)
  {
    ...
    pkmedian_hizs[pid] = new CheckedCompletableFuture<Double>();
    ...
    pkmedian_hizs[pid].complete(myhiz);

    double hiz = 0.0;
    for (int i = 0; i < NPROC; i++)
      hiz += (Double) pkmedian_hizs[i].join();
    double loz = 0.0;
    ...
    if (pid == 0) {
      selectfeasible_fast(kmin, pid);
      for (int ii = 0; ii < p.length; ii++)
        is_center[p[ii].assign] = true;

      p_masterCondition = new Channel<>();
      p_masterCondition_f = p_masterCondition.fromLatest();
    }

    barrier.getParty(pid).arrive();

    Iterator<Void> masterCondition = p_masterCondition_f.iterator();
