...
static const unsigned int DAC_ARITH_BASECASE = (1 << POWER);
static const unsigned int MATMUL_THRESH = (1 << (POWER + 4));
...
static void mm_additive_base(REAL *C, REAL *A, REAL *B, int n) {
  //printf("doing mm_additive_base once \n");
  REAL *ptrToA = A;
  REAL *ptrToB = B;
...
void print_matrix(REAL *M, int n){
    ...
    for(i = 0; i < n; i++){
        for(j = 0; j < n; j++){
            printf("%d ", (int)M[block_convert(i,j)]);
        }
        printf("\n");
    }
    printf("\n");
