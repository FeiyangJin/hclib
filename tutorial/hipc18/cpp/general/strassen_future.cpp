...
static int BASE_CASE = (int) pow(2.0, (double) POWER);
...
static void init_matrix(int n, REAL *A) {

  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n; ++j) {
      A[block_convert(i,j)] = 1;
...
void mm_dac_z(REAL *C, REAL *A, REAL *B, int n, bool add) {
  ...
  mm_dac_z(C11, A11, B11, n >> 1, add);
  mm_dac_z(C12, A11, B12, n >> 1, add);
  mm_dac_z(C21, A21, B11, n >> 1, add);
  mm_dac_z(C22, A21, B12, n >> 1, add);

  mm_dac_z(C11, A12, B21, n >> 1, true);
  mm_dac_z(C12, A12, B22, n >> 1, true);
  mm_dac_z(C21, A22, B21, n >> 1, true);
  mm_dac_z(C22, A22, B22, n >> 1, true);
...
static void final_add(REAL *C11, REAL *C12, REAL *C21, REAL *C22, 
	...
	final_add(C11_0, C12_0, C21_0, C22_0, M2_0, M5_0, T1_0, n >> 1);
	final_add(C11_1, C12_1, C21_1, C22_1, M2_1, M5_1, T1_1, n >> 1);
	final_add(C11_2, C12_2, C21_2, C22_2, M2_2, M5_2, T1_2, n >> 1);
...
void strassen_z(REAL *C, REAL *A, REAL *B, int n) {
  ...
  strassen_z(M2, A11, B11, new_n);  // P1, store in M2
  strassen_z(M5, S1, S5, new_n);    // P5, store in M5 
  strassen_z(T1, S2, S6, new_n);    // P6, store in T1 
  strassen_z(C22, S3, S7, new_n);   // P7, store in C22 
  strassen_z(C11, A12, B21, new_n); // P2, store in C11
  strassen_z(C12, S4, B22, new_n);  // P3, store in C12
...
void print_matrix(REAL *M, int n){
    ...
    for(i = 0; i < n; i++){
        for(j = 0; j < n; j++){
            printf("%.1f and index is %d", M[block_convert(i,j)],block_convert(i,j));
        }
        printf("\n");
    }
    printf("\n");
...
int main(int argc, char *argv[]) {
    ...
    for(int i=0; i<n*n; i++){
      printf("the %dth element of A is %f \n",i,A[i]);
    }
